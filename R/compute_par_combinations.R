...
#'
#' @param par_input_list List of parameters values to be processed. Name of
...
compute_par_combinations <- function(par_input_list, parameter_method){
  ...
  if (parameter_method == "all_combinations"){
    ...
    out <- expand.grid(par_input_list)
  }
  ...
  if (parameter_method == "monte_carlo"){
    ...
    # Data checks
    stopifnot(sapply(seq_len(k), function(x,y) length(y[[x]])==3, y=par_input_list)) # Check that there are 3 values for each parameter (presumably a min, max and n)
    stopifnot(sapply(seq_len(k), function(x,y) y[[x]][3]==y[[1]][3], y=par_input_list)) # Check that total n is equal for all parameters.

    # Inputs
    k <- length(par_input_list)
    min_par <- sapply(seq_len(k), function(x,y) y[[x]][1], y=par_input_list)
