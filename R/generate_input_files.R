...
#'
#' @export
generate_input_files <- function(input_hdr_list, option_sets_def, input_dated_seq_list, parameter_method, world_hdr_prefix, world_file){

  # ---------------------------------------------------------------------
  # Generate def files

  if (is.null(option_sets_def)==FALSE){
    # Step though each def file
    for (aa in seq_along(option_sets_def)){

      # Step through each unique parameter set and make def file
      for (bb in seq_along(option_sets_def[[aa]]$group_id)){

        change_def_file(def_file = names(option_sets_def)[aa],
                        par_sets = dplyr::select(option_sets_def[[aa]], -group_id)[bb,],
                        file_name_ext = as.character(option_sets_def[[aa]]$group_id[bb]))
      }
      print(paste("New def files printed for file", names(option_sets_def)[aa]))
    }
  }
