...
run_rhessys_single <- function(
  ...
  if (any(!req_rhessys_input %in% names(input_rhessys))) {
    stop(paste0(
      "Missing rhessys_input(s):",
      names(input_rhessys)[!req_rhessys_input %in% names(input_rhessys)]
    ))
  }
  # Following inputs must exist to run
  if (!file.exists(input_rhessys$rhessys_version) && is.null(docker_image)) { # only matters if not in container
    stop(paste(
      "RHESSys version",
      input_rhessys$rhessys_version,
      "does not exist."
    ))
  }
  if (!file.exists(input_rhessys$world_file)) {
    stop(paste("World file", input_rhessys$world_file, "does not exist."))
  }
  if (!file.exists(input_rhessys$flow_file)) {
    stop(paste("Flow table", input_rhessys$flow_file, "does not exist."))
  }
  # auto generate output folder
  if (
    !is.null(input_rhessys$output_folder) &&
      !dir.exists(input_rhessys$output_folder)
  ) {
  ...
  if (!is.character(hdr_files)) {
    ...
    if (!is.null(clim_base)) {
      not_check <- c(not_check, 'base_stations')
    }

    if (
      any(!file.exists(unlist(hdr_files[!names(hdr_files) %in% not_check])))
    ) {
      stop(
        "Def file(s) '",
        unlist(hdr_files[!names(hdr_files) %in% not_check])[
          !file.exists(unlist(hdr_files[!names(hdr_files) %in% not_check]))
        ],
  ...
  if (!is.null(def_pars)) {
    if (is.character(hdr_files)) {
      stop(
        "Cannot use an existing unmodified hdr file to reference modified def files - please use the IOin_hdr() function to input hdr file data in R."
      )
    }

    if (!is.data.frame(def_pars)) {
      def_pars_df <- data.frame(matrix(
        unlist(def_pars),
        nrow = length(def_pars),
        byrow = T
      ))
    } else {
    ...
    if (any(!unique(def_pars_df$X1) %in% unlist(hdr_files))) {
      stop(
        "Def file '",
        unique(def_pars_df$X1)[!unique(def_pars_df$X1) %in% unlist(hdr_files)],
        "' being modified is not included in the input header list of def files (input_hdr)."
      )
    }
    ...
    for (f in unique(def_pars_df$X1)) {
      ...
      new_file <- change_def_file(
        def_file = f,
        par_sets = def_par_subset,
        file_name_ext = runID
      )
  ...
  if (!is.null(cmd_pars)) {
    ...
    cmd_pars <- lapply(cmd_pars, function(X) {
      if (is.null(X)) {
        X <- 1
      }
      return(X)
  ...
  if (!is.null(clim_base)) {
    ...
    return_file_list &&
      !is.null(hdr_files) &&
      is.list(hdr_files) &&
      !is.null(hdr_files$base_stations)
  ) {
  ...
  if (is.list(hdr_files)) {
    world_hdr_name_out <- make_hdr_file(
      input_rhessys,
      hdr_files,
      def_files,
      runID
  ...
  if (!is.null(output_filter)) {
    ...
    if (
      !is.null(tec_data) && is.data.frame(tec_data) && !is.null(tec_data$name)
    ) {
      filter_timesteps <- write_output_filter(
        output_filter,
        runID,
      ...
      filter_tec_names <- unlist(lapply(filter_timesteps, function(x) {
        tec_print_list[[x]]
      }))
      ...
      if (length(missing_from_tec) > 0) {
        warning(paste0(
          "The following output filter timesteps are not present in the tec file: ",
          paste(missing_from_tec, collapse = ", "),
          ". This may result in no output being generated for those timesteps."
        ))
      }
      if (length(missing_from_filter) > 0) {
        warning(paste0(
          "The following tec print events are not present in the output filter: ",
          paste(missing_from_filter, collapse = ", "),
          ". This may result in no output being generated for those timesteps."
        ))
      }
    }
    ...
    output_path <- file.path(
      input_rhessys$output_folder,
      input_rhessys$output_filename
    )
    if (!is.null(runID)) {
  ...
  if (write_run_metadata & is.null(runID)) {
    run_info_file <- write_run_info(
      rhessys_version = input_rhessys$rhessys_version,
  ...
  if (write_run_metadata & is.null(runID)) {
    ...
    cat(
      paste0("     Successful Output?:\t", paste0(fex, collapse = ", "), "\n"),
      file = file,
      sep = ""
    )
    close(file)
