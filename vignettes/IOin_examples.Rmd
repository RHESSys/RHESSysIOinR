...
run_rhessys_multi(
  ...
  hdr_files = input_hdr,
  tec_data = input_tec_data,
  def_pars = input_def_pars_multi,
  output_filter = output_filter
)
```

### IOin_def_pars_latin_hypercube
This function generates parameters according to [latin hypercube sampling] (https://en.wikipedia.org/wiki/Latin_hypercube_sampling). In this example, compared to the previous one, the parameter ranges are passed to the `IOin_def_pars_latin_hypercube` (note the `c` instead of `runif` for the third element of each list), instead of being first evaluated.
